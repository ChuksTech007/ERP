...
export function balanceSheet({ db = getDb(), upTo = null } = {}) {
  ...
  /* Reported with the same `amountKobo` key the profit and loss uses. They
   * arrive from the trial balance as `balanceKobo`, and the mismatch put a
   * literal "NaN" on every line of the owner's balance sheet while the totals
   * underneath stayed correct — the kind of wrong that looks like the whole
   * report cannot be trusted. */
  const pick = (type) =>
    accounts
      .filter((a) => a.type === type && a.balanceKobo !== 0)
      .map((a) => ({ ...a, amountKobo: a.balanceKobo }));

  const assets = pick('asset');
  const liabilities = pick('liability');
  const equity = pick('equity');

  const assetsKobo = assets.reduce((sum, a) => sum + a.amountKobo, 0);
  const liabilitiesKobo = liabilities.reduce((sum, a) => sum + a.amountKobo, 0);
  const equityKobo = equity.reduce((sum, a) => sum + a.amountKobo, 0);
