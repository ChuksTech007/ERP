...
export function postSale(
  { saleId, invoiceNumber, totalKobo, depositAppliedKobo = 0, isFraming = true, incomeSplit = null, date, userId },
  { db = getDb() } = {}
) {
  /* A framing job earns in one place. A counter sale can span two — a
   * ready-made frame and a portrait reprint on the same receipt — so the
   * caller may hand over the split instead. Rolling both into one account
   * would make it impossible to tell later which side of the shop earns. */
  const credits =
    incomeSplit && incomeSplit.length > 0
      ? incomeSplit
      : [{ accountCode: isFraming ? ACCT.FRAMING_SALES : ACCT.PORTRAIT_SALES, amountKobo: totalKobo }];

  const creditedKobo = credits.reduce((sum, c) => sum + c.amountKobo, 0);
  if (creditedKobo !== totalKobo) {
    throw new Error(
      `Income split of ${creditedKobo} does not add up to the invoice total of ${totalKobo}.`
    );
  }
